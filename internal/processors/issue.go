...
func (p *IssueProcessor) ProcessCreate(bctx bahamut.Context) (err error) {
	...
	if req.AuthorizeRequestID != "" {
		authorizeContext, oauthClient, oauthApplication, err = p.loadAuthorizeIssueContext(bctx.Context(), req, req.AuthorizeRequestID, source)
		if err != nil {
			return err
		}
	}
...
func validateAuthorizeIssueSource(req *api.Issue, source elemental.Identifiable, app *api.OAuthApplication) error {
	...
	if source == nil {
		return fmt.Errorf("unable to load source for attribute-based matching")
	}
...
func (p *IssueProcessor) completeAuthorizeIssue(
	...
	if err != nil {
		return fmt.Errorf("unable to complete authorization: %w", err)
	}
