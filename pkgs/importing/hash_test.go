...
func Test_sanitize(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// ref: a single nested object must be recursively sanitized.
			// Autogenerated/readonly fields (ID) and zero/default values
			// on the nested object must be stripped, real content kept.
			"with ref nested object",
			func(*testing.T) args {
				obj := testmodel.NewList()
				obj.Name = "list"
				obj.Ref = testmodel.NewTask()
				obj.Ref.ID = "should be removed"
				obj.Ref.Name = "task1"
				return args{
					obj.Identity().Name,
					obj,
		...
		{
			// refList: each element of the list must be recursively
			// sanitized just like a top-level object.
			"with refList whose elements must be sanitized",
			func(*testing.T) args {
				obj := testmodel.NewList()
				obj.Name = "list"

				t1 := testmodel.NewTask()
				t1.ID = "should be removed"
				t1.Name = "task1"

				t2 := testmodel.NewTask()
				t2.ID = "should be removed"
				t2.Name = "task2"

				obj.RefList = testmodel.TasksList{t1, t2}
				return args{
					obj.Identity().Name,
					obj,
					testmodel.Manager(),
			...
			map[string]any{
				"name": "list",
				"refList": []any{
					map[string]any{"name": "task1"},
					map[string]any{"name": "task2"},
				},
			},
			false,
			nil,
		...
		{
			// refMap: each value of the map must be recursively sanitized
			// while the map keys are preserved.
			"with refMap whose values must be sanitized",
			func(*testing.T) args {
				obj := testmodel.NewList()
				obj.Name = "list"
				...
				obj.RefMap = map[string]*testmodel.Task{
					"a": t1,
				}
				return args{
					obj.Identity().Name,
