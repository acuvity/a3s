...
func Hash(obj Importable, manager elemental.ModelManager) (string, error) {
	...
	data, err := sanitize(obj.Identity().Name, obj, manager)
	if err != nil {
...
func getIdentifiableOrDetached(name string, manager elemental.ModelManager) any {

	if obj := manager.IdentifiableFromString(name); obj != nil {
		return obj
	}

	return manager.DetachedFromString(name)
}

func sanitize(restName string, obj elemental.AttributeSpecifiable, manager elemental.ModelManager) (map[string]any, error) {
	...
	pobj := getIdentifiableOrDetached(restName, manager).(elemental.AttributeSpecifiable)
	...
	for k, v := range data {
		...
		spec := obj.SpecificationForAttribute(strings.ToLower(k))

		if spec.Name != "namespace" && (!spec.Exposed || spec.ReadOnly || spec.Autogenerated) {
			delete(data, k)
			continue
		}
		...
		if spec.Type == "ref" {
			...
			if vv := getIdentifiableOrDetached(spec.SubType, manager); vv != nil {
...
func hash(data map[string]any) (string, error) {
	...
	return fmt.Sprintf("%x", sha256.New().Sum(fmt.Appendf([]byte{}, "%d", h))), nil
}
